"""Meta Demucs v1, v2, v3, and v4 (HTDemucs) separation model."""
...
class SeparateDemucs(SeparateAttributes):
    def seperate(self):
        samplerate = 44100
        source = None
        model_scale = None
                ...
                def secondary_save(sec_stem_name, source, raw_mixture=None, is_inst_mixture=False):
                    ...
                    if not isinstance(secondary_source, np.ndarray):
                        if self.is_demucs_combine_stems:
                            source = list(source)
                            if is_inst_mixture:
                                source = [i for n, i in enumerate(source) if n not in [self.demucs_source_map[self.primary_stem], self.demucs_source_map[VOCAL_STEM]]]
                            else:
